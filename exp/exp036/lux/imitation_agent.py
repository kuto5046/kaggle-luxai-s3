...
def policy_map_to_action(
    ...
    assign_greedy_actions_sap(
        actions, obs, available_unit_ids, unit_positions, policy_map, sap_map, stochastic, env_cfg
    )
    ...
    assign_actions_with_flow(
        actions, obs, next_available_unit_ids, unit_positions, policy_map, sapped_unit_next_pos_set, overlap_penalty
    )
    return actions


def create_dummy_policy(action_map: np.ndarray, obs: dict[str, Any], team_id: int, env_cfg: EnvParams) -> np.ndarray:
    # その行動確率が高くなるように適当にpolicyを設定
    ...
    for x, y in unit_positions:
        ...
        policy_map[:, y, x] = np.log(0.02)
        policy_map[action, y, x] = np.log(0.9)
    return policy_map
...
class ILAgent:
    def __init__(self, model: nn.Module, cfg: dataclass) -> None:
        self.model = model
        self.env_cfg = EnvParams()
        self.cfg = cfg
        # n_stack分のstateを保持するqueue
        self.stack_states = deque(maxlen=cfg.n_stack)
        self.stack_global_states = deque(maxlen=cfg.n_stack)
...
class MinimumCostFlow:
    ...
    def flow(self, s, t, flow, timeout=5.0):
        n = self.n
        g = self.edges
...
def assign_actions_with_flow(
    actions, obs, available_unit_ids, unit_positions, policy_map, sapped_unit_next_pos_set, overlap_penalty
):
...
# def assign_greedy_actions(
#     actions, available_unit_ids, unit_positions, policy_map, point_map, opp_unit_positions, prev_opp_unit_positions, stochastic, env_cfg
# ):
#     for unit_id in available_unit_ids:
#         unit_pos = unit_positions[unit_id]
#         x, y = unit_pos
...
#                     unit_pos, opp_unit_positions, env_cfg["unit_sap_range"]
#                 )
...
def assign_greedy_actions_sap(
    ...
    for unit_id in available_unit_ids:
        ...
        if action == Action.SAP:
            ...
            sap_poses = []
            sap_policies = []
            for dx in range(-env_cfg.unit_sap_range, env_cfg.unit_sap_range + 1):
                for dy in range(-env_cfg.unit_sap_range, env_cfg.unit_sap_range + 1):
                    sap_pos = (x + dx, y + dy)
                    if in_map(sap_pos):
            ...
            sap_infos.append(SingleSapInfo(sap_poses, sap_policies, next_policy, next_action_info, unit_id, unit_pos))
