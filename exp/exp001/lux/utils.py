...
def extract_state(obs: dict[str, Any], target_team_idx: int, state_space_size: int = 15) -> np.ndarray:
    ...
    state_map[2] = np.array(obs["sensor_mask"][target_team_idx])

    for k, (i, j) in enumerate(obs["relic_nodes"]):
        state_map[3, i, j] = 1
        state_map[4, i, j] = obs["relic_nodes_mask"][k]
    ...
    for team_idx in range(2):
        ...
        for unit_idx in range(EnvParams.max_units):
            ...
            if team_idx == target_team_idx:
                state_map[5, pos[0], pos[1]] += 1  # 重複はそんなに発生しないだろうということで正規化はしない
                state_map[6, pos[0], pos[1]] += unit_energy / EnvParams.max_unit_energy
                state_map[7, pos[0], pos[1]] = mask
            else:
                state_map[8, pos[0], pos[1]] += -1
                state_map[9, pos[0], pos[1]] += -unit_energy / EnvParams.max_unit_energy
                state_map[10, pos[0], pos[1]] = mask  # TODO: 敵のマスク情報ってわかるんだっけ？

    # game state
    state_map[11] = obs["match_steps"] / EnvParams.max_steps_in_match  # そのマッチの進行度
    state_map[12] = obs["steps"] // EnvParams.max_steps_in_match  # 何試合目か
    state_map[13] = (obs["team_points"][target_team_idx] - obs["team_points"][opponent_team_idx]) / 100
    state_map[14] = (
...
def extract_action(step_info: dict[str, Any], target_team_idx: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for unit_idx in range(num_units):
        ...
        if actions:
            # maskされているところは必ずaction=0になってるはず→TODO: というわけではない?
            # if mask:
            #     print(actions[unit_idx])
...
def get_valid_policy_map(obs: dict[str, Any], team_idx: int, action_space_size: int = 6) -> np.ndarray:
    validate_policy_map = np.zeros((action_space_size, EnvParams.map_width, EnvParams.map_height), dtype=np.float32)
    for unit_idx in range(EnvParams.max_units):
...
def can_move(pos: tuple[int, int], energy: int, dir: int) -> bool:
    ...
    3. TODO: ASTEROID_TILEタイルが存在する(一旦skip)
