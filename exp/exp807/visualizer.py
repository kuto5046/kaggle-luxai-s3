...
def extract_results(
    ...
    for step_idx in range(len(steps) - 1):  # 505でdoneとなるため-1
        ...
        gt_sap_map = extract_sap_map(
            next_actions,
            opp_next_actions,
            gt_obs,
            target_team_id,
            env_params.max_sap_range,
            0.5,
        )
...
def visualize_sap_map(
    ...
    n_units = len(unit_positions)
    n_cols = 4
    n_rows = (2 * n_units + n_cols - 1) // n_cols

    fig = make_subplots(
        rows=n_rows,
        cols=n_cols,
        subplot_titles=[
            f"{unit_id=} \n position={(x, y)}" if i % 2 == 0 else f"gt {unit_id=} \n position={(x, y)}"
            for unit_id, (x, y) in enumerate(unit_positions)
            for i in range(2)
        ],
    )
    for unit_id, (x, y) in enumerate(unit_positions):
        row = (2 * unit_id) // n_cols + 1
        col = (2 * unit_id) % n_cols + 1
        row_gt = (2 * unit_id + 1) // n_cols + 1
        col_gt = (2 * unit_id + 1) % n_cols + 1
        fig.add_trace(go.Heatmap(z=unit_sap_maps[unit_id], showlegend=False), row=row, col=col)
        fig.add_trace(go.Heatmap(z=gt_sap_map[Sap.SAP_MAP, unit_id], showlegend=False), row=row_gt, col=col_gt)
    # レイアウトの調整
    fig.update_layout(
        height=200 * n_rows,
        # width=800,
