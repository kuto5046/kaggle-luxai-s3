...
def no_sap(actions: list[list[int]]) -> bool:
    """
    アクションの中にSAPが一つもなければTrueを返す

    Args:
        actions: アクション配列

    Returns:
        bool: SAPが一つもなければTrue
    """
    return np.all(np.array(actions)[:, 0] != Action.SAP)
    ...
    def _process_episode(self, row) -> tuple[str, int, int]:
        ...
        with h5py.File(self.feature_dir / f"temp_{episode_id}.h5", "w") as out_f:
            ...
            for idx in range(len(steps) - 1):  # 505でdoneとなるため-1
                ...
                if no_sap(next_step_info[target_team_id]["action"]):
                    continue
