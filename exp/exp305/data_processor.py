...
class Config:
    ...
    n_splits: int = 5
    stratify: bool = False
    root_dir: Path = Path("/root/kaggle-luxai-s3")
    input_dir: Path = root_dir / "input/kaggle_datasets/luxai-s3-frog-parade-42704976-ver2"
    episode_dir: Path = input_dir / "episodes"
    episode_path: Path = input_dir / "episodes.csv"
...
class DataProcessor:
    ...
    def _process_episode(self, row) -> tuple[str, int, int]:
        ...
        final_step_in_match = [
            (i_match + 1) * EnvParams.max_steps_in_match + i_match
        ...
        use_steps = (
            match_confirmed_step if self.cfg.ignore_after_3_wins and match_confirmed > 0 else final_step_in_match[-1]
        )  # 504
    ...
    def preprocess(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        with h5py.File(self.feature_dir / "episodes.h5", "w") as out_f:
            for episode_id in tqdm(valid_ids, total=len(valid_results)):
