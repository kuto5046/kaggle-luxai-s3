...
class LaxLitModel(LightningModule):
    ...
    def freeze(self):
        ...
        #     param.requires_grad = False
        # for param in self.model.sap_net.parameters():
        #     param.requires_grad = False
...
class LuxResNetModel(nn.Module):
    def __init__(
        self,
        state_space_size: int,
        global_state_space_size: int,
        ...
        self.inc = DoubleConv((state_space_size + global_state_space_size) * n_stack, self.hidden_dim, res=False)
...
class MaskedFocalTverskyLoss(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        focal_loss = torch.maximum((1 - Tversky), torch.tensor(1e-10).to(Tversky.device)) ** self.gamma
