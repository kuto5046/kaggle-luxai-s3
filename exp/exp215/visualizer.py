...
def main():
    ...
    if json_load is not None:
        ...
        try:
            ...
            episode_id = "unknown"
            target_team_id = 0
            st.error("EpisodeId or TeamNames not found in json file")
            st.error("Assuming local mode")
        ...
        if replay_submit:
            ...
            gt_env_params = EnvParams(**json_load["params"])
        env_params = gt_env_params
        with st.expander("Env Params"):
            st.write(gt_env_params)

        results = extract_results(json_load, target_team_id, env_params, episode_id, cfg, replay_submit)
