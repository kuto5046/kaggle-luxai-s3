...
class EpisodeStore:
    def __init__(
        ...
        self._relic_found_all = False
        self._relic_candidate_map = np.ones((EnvParams.map_height, EnvParams.map_width), dtype=np.float32)
    ...
    @property
    def relic_candidate_map(self) -> np.ndarray:
        return self._relic_candidate_map.copy()
    ...
    def update(self, obs: dict[str, Any], actions: np.ndarray) -> None:
        ...
        self._update_relic_candidate_map(obs)
    ...
    def _update_relic_map(self, obs: dict[str, Any]) -> None:
        ...
        if self._relic_found_all:
            # raise ValueError("new relic found, but self._relic_found_all is True")
            # 対角線上にrelicがある場合と、relicが同じ場所に重ねて出現する場合の処理が甘いので
            # 一旦 ValueError ではなく print に変更
            print(f"new relic found, but self._relic_found_all is True: {self.episode_id=}", file=sys.stderr)
        ...
        for x, y in new_relic_nodes:
            ...
            if self._relic_candidate_map[y, x] == 0:
                print(f"new relic found, but relic_candidate_map is 0: {self.episode_id=}", file=sys.stderr)
    ...
    def _update_relic_candidate_map(self, obs: dict[str, Any]) -> None:
        if self._relic_found_all:
            self._relic_candidate_map[:] = 0
            return

        match_count = obs["steps"] // (EnvParams.max_steps_in_match + 1)

        num_relic = len(self._relic_nodes)
        max_relic = 2 * min(3, match_count + 1) - 1  # 対角線上にrelicがある場合があるので1減らす

        # Relic が十分数見つかれば他のセルは0にする
        if num_relic >= max_relic:
            self._relic_candidate_map[:] = 0
        elif (obs["match_steps"] <= 50) and (match_count < 3):
            self._relic_candidate_map[:] = 1
            return

        # 探索済みの範囲をマスク
        sensor_mask = np.array(obs["sensor_mask"]).T
        self._relic_candidate_map = np.where(sensor_mask, 0, self._relic_candidate_map)
        sensor_mask_opposite = (sensor_mask.T)[::-1, ::-1]
        self._relic_candidate_map = np.where(sensor_mask_opposite, 0, self._relic_candidate_map)

        # 全てのrelicが見つかったか判定
        if self._relic_candidate_map.sum() == 0:
            if (num_relic < max_relic) or (num_relic >= EnvParams.max_relic_nodes - 1):
                self._relic_found_all = True
...
def extract_gt_state(obs: dict[str, Any], target_team_id: int) -> np.ndarray:
    ...
    state_map[State.RELICS_CANDIDATE] = state_map[State.RELICS].copy()
...
def extract_state(obs: dict[str, Any], target_team_id: int, episode_store: EpisodeStore) -> np.ndarray:
    ...
    state_map[State.RELICS_CANDIDATE] = episode_store.relic_candidate_map
