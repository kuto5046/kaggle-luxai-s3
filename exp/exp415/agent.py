        ...
        while flow > 0:
            ...
            while que:
                ...
                for i, edge in enumerate(g[v]):
                    w, cap, cost, rev, _ = edge
                    if cap > 0:
                        nd = dist[v] + cost + h[v] - h[w]
                        if nd < dist[w] - eps:
                            dist[w] = nd
                            prevv[w] = v
                            preve[w] = i
...
class Agent:
    ...
    def act(self, step: int, obs, remainingOverageTime: int = 60):
        ...
        # available_unit_idsからsapしたユニットとenergyが0のユニットを除外
        next_available_unit_ids = [unit_id for unit_id in available_unit_ids if actions[unit_id][0] != Action.SAP]
        # next_available_unit_ids = [
        #     unit_id for unit_id in next_available_unit_ids if obs["units"]["energy"][self.team_id][unit_id] > 0
        # ]
        ...
        #     actions, available_unit_ids, policy_per_unit
        # )
    ...
    def _assign_actions_with_flow(
        ...
        for unit_id in available_unit_ids:
            ...
            if policy.sum() > 0:  # policyの合計が0の場合はスキップ
                ...
                print(f"policy sum is 0: {policy=}", file=sys.stderr)
            ...
            for action in range(len(Action)):
                ...
                valid_actions.append({"action_id": action, "next_pos": next_pos, "score": score, "sap_pos": None})
    ...
    def _assign_greedy_actions(self, actions, available_unit_ids, policy_per_unit):
        for unit_id in available_unit_ids:
...
def test_mincost_flow():
    flow = MinimumCostFlow(7)
    flow.add_edge(0, 2, 1, 0, -1)
    flow.add_edge(0, 3, 1, 0, -1)
    flow.add_edge(0, 4, 1, 0, -1)
    flow.add_edge(5, 1, 1, 0, -1)
    flow.add_edge(6, 1, 2, 2.0, -1)
    flow.add_edge(
        2,
        5,
        1,
        2.020522135603997e-06,
        {"action_id": 0, "next_pos": (0, 0), "score": 2.020522135603997e-06, "sap_pos": None},
    )
    flow.add_edge(
        2,
        6,
