...
class LuxAugmentStandardize(LuxAugmentBase):
    ...
    def __call__(self, inputs: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
        ...
        action_target_mask = inputs["action_target_mask"].copy()
        ...
        if do_flip:
            ...
            action_target_mask = np.flip(action_target_mask, axis=(0, 1)).copy()
        ...
        inputs["action_target_mask"] = action_target_mask
        return inputs
...
class LuxAugmentTranspose(LuxAugmentBase):
    ...
    def __call__(self, inputs: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
        ...
        if random.random() < self.p:
            ...
            action_target_mask = np.transpose(action_target_mask, (1, 0)).copy()
...
class LaxLitModel(LightningModule):
    ...
    def _share_step(self, batch: Any, mode: str = "train") -> torch.Tensor:
        ...
        action_target_mask = batch["action_target_mask"] > 0
        policy_loss = self.criterion1(policy_preds, policy_targets, action_target_mask)
