...
def extract_action(actions: np.ndarray, obs: dict[str, Any], target_team_id: int) -> np.ndarray:
    action_map = np.zeros((4, EnvParams.map_width, EnvParams.map_height), dtype=np.float32)
    # unit state
    ...
    for unit_id in available_unit_ids:
        energy = obs["units"]["energy"][target_team_id][unit_id]
        # energy0のunitがcenterであることを学習しても意味がないので無視
        if energy == 0:
            continue
        x, y = unit_positions[unit_id]
        action_map[3, y, x] = 1
...
def get_valid_policy_per_unit(obs: dict[str, Any], team_id: int, episode_store: EpisodeStore) -> np.ndarray:
    ...
    for unit_id in available_unit_ids:
        ...
        x, y = pos
        if x == -1 or y == -1:
            print(f"{obs['steps']=} {team_id=} {unit_id=} {pos=}", file=sys.stderr)
            assert False
        energy = obs["units"]["energy"][team_id][unit_id]
