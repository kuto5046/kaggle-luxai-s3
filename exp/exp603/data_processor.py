...
class Config:
    ...
    remove_episodes = []
    ignore_after_3wins = True

    _suffix = "_ignore_after_3wins" if ignore_after_3wins else ""
    feature_dir: Path = root_dir / f"output/feature_store/aDg4b_42683570{_suffix}"  
...
class DataProcessor:
    ...
    def read_data(self) -> pl.DataFrame:
        ...
        episode_df = episode_df.filter(~pl.col("EpisodeId").is_in(self.cfg.remove_episodes))
        return episode_df
    ...
    def _process_episode(self, row) -> tuple[str, int, int]:
        ...
        with h5py.File(self.feature_dir / f"temp_{episode_id}.h5", "w") as out_f:
            ...
            if self.cfg.ignore_after_3wins:
                num_valid_steps = min((np.argmax(np.cumsum(match_results) == 3) + 1) * 101, 504)  # 3勝するまでのstep数
            else:
                num_valid_steps = (EnvParams.max_steps_in_match + 1) * 5 - 1
            
            for step_idx in range(num_valid_steps + 1):  # 505でdoneとなるため-1                
                ...
                match_idx = max(0, (obs["steps"] - 1) // (EnvParams.max_steps_in_match + 1))  # fix
                is_win = match_results[match_idx]  # TODO: fix
                episode_win_group.create_dataset(f"{step_idx}", data=is_win)

        return str(episode_id), num_valid_steps + 1, target_team_id, is_win

    def preprocess(self, df: pl.DataFrame) -> pl.DataFrame:
        if DEBUG:
            results = []
            for row in tqdm(df.iter_rows(named=True), total=len(df)):
                results.append(self._process_episode(row))
        else:
            # 並列処理の実行
