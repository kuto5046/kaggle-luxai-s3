...
class Space:
    ...
    def _update_relic_map(self, obs, team_id, team_reward):
        for relic_id, (mask, xy) in enumerate(zip(obs["relic_nodes_mask"], obs["relic_nodes"])):
            if mask:
    ...
    def _update_reward_status_from_reward_results(self):
        ...
        for result in Global.REWARD_RESULTS:
            ...
            if reward == 0:
                ...
                print(
                    f"Something wrong with reward result: {result}" ", this result will be ignored.",
                    file=stderr,
                )
        ...
        def set_task(ship, relic_node, can_pause):
            ...
            if target == ship.coordinates and not can_pause:
                target, _ = find_closest_target(
                    ship.coordinates,
                    [n.coordinates for n in self.space if n.explored_for_reward and n.is_walkable],
                )
